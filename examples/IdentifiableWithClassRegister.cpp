...
{
    ANAX_IDENTIFIABLE(Cat)
};

class Dog : public Animal
{
    ANAX_IDENTIFIABLE(Dog)
};

class Lion : public Animal
{
    ANAX_IDENTIFIABLE(Lion)
};

class Tiger : public Animal
{
    ANAX_IDENTIFIABLE(Tiger)
};
...
{
    return ac::create<Animal>(possibleRandomElements[rand() % possibleRandomElements.size()]);
}
...
int main(int argc, char* argv[])
{
    ...
    for(size_t i = 0; i < amountOfAnimalsToCreate; ++i)
    {
        ...
        std::cout << randomAnimal->getClass().getName() << '\n';
